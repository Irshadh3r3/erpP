...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $discount_percentage = (float)$_POST['discount'];
    $tax = (float)$_POST['tax'];
    ...
    $discount_amount = ($subtotal * $discount_percentage) / 100; // Convert percentage to amount
...
$stmt->bind_param("sisdddddsssi", $invoice_number, $customerId, $sale_date, $subtotal, $discount_amount, $tax, $total_amount, $paid_amount, $payment_status, $payment_method, $notes, $_SESSION['user_id']);
            $stmt->execute();
            
            $saleId = $conn->insert_id;

            // If a paid amount was provided, insert a corresponding payment record
            if ($paid_amount > 0) {
                $paymentNotes = 'Initial payment during invoice creation';
                $payStmt = $conn->prepare("INSERT INTO payments (invoice_id, payment_date, payment_amount, payment_method, reference_number, notes, user_id, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, NOW())");
                // reference_number not provided here; insert empty string
                $ref = '';
                $payStmt->bind_param('isdsssi', $saleId, $sale_date, $paid_amount, $payment_method, $ref, $paymentNotes, $_SESSION['user_id']);
                $payStmt->execute();
            }
            ...
            while ($item = $items->fetch_assoc()) {
                // allow fractional quantities when recording sales_items
                $stmt->bind_param("iiddd", $saleId, $item['product_id'], $item['quantity'], $item['unit_price'], $item['subtotal']);
                $stmt->execute();
...
function calculateInvoiceTotal() {
    ...
    document.getElementById('display_discount').textContent = discountPercentage.toFixed(2) + '% - Rs. ' + discountAmount.toFixed(2);
