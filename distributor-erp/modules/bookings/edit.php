    ...
    if (empty($errors)) {
        ...
        try {
            ...
            foreach ($valid_items as $item) {
                ...
                $stmt->bind_param("iiddd", $bookingId, $item['product_id'], $item['quantity'], $item['unit_price'], $item['subtotal']);
                $stmt->execute();
            }
                    ...
                    <div class="col-span-5">
                        <select name="product_id[]" class="product-select"
                            ...
                            <?php endwhile; ?>
                                ...
                                <div id="variety_<?php echo $rowIndex; ?>" class="text-xs text-gray-500 mt-1"></div>
                    </div>
...
function addProductRow() {
    ...
    row.innerHTML = `
        <div class="col-span-5">
                    ...
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                <option value="">Select Product</option>
                ${products.map(p => `<option value="${p.id}" data-price="${p.selling_price}" data-stock="${p.stock_quantity}" data-variety="${p.variety ?? ''}">${p.name}${p.variety ? ' - ' + p.variety : ''} (Stock: ${p.stock_quantity})</option>`).join('')}
            </select>
    ...
    // initialize Tom Select on newly added select
    setTimeout(function () {
...
// Populate variety and price for any pre-filled rows, then calculate totals
