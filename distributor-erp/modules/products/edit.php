...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Allow fractional quantities when unit is ml/kg/ltr
    ...
    if (empty($errors)) {
        // If category_id is NULL we need to set the column to SQL NULL instead of binding 0
        if ($category_id === null) {
            $stmt = $conn->prepare("UPDATE products SET sku = ?, name = ?, category_id = NULL, variety = ?, description = ?, unit = ?, purchase_price = ?, selling_price = ?, stock_quantity = ?, reorder_level = ?, barcode = ? WHERE id = ?");
            $stmt->bind_param("sssssdddisi", $sku, $name, $variety, $description, $unit, $purchase_price, $selling_price, $stock_quantity, $reorder_level, $barcode, $productId);
        } else {
            $stmt = $conn->prepare("UPDATE products SET sku = ?, name = ?, category_id = ?, variety = ?, description = ?, unit = ?, purchase_price = ?, selling_price = ?, stock_quantity = ?, reorder_level = ?, barcode = ? WHERE id = ?");
            $stmt->bind_param("ssisssdddisi", $sku, $name, $category_id, $variety, $description, $unit, $purchase_price, $selling_price, $stock_quantity, $reorder_level, $barcode, $productId);
        }
    ...
    $_POST = $product;
    // ensure variety has a place in prefilled data (if column doesn't exist in DB dump it will be null)
    if (!isset($_POST['variety']) && isset($product['variety'])) {
        $_POST['variety'] = $product['variety'];
    }
}
