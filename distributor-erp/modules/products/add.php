...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $stmt = $conn->prepare("INSERT INTO products (sku, name, category_id, description, unit, purchase_price, selling_price, stock_quantity, reorder_level, barcode) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("ssissddiis", $sku, $name, $category_id, $description, $unit, $purchase_price, $selling_price, $stock_quantity, $reorder_level, $barcode);
